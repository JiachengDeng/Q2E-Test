...
class CorrAttention(Module):
    ...
    def forward(self, corr_map, pos_emb):
        ...
        pos_emb = torch.repeat_interleave(pos_emb, self.num_heads, dim=1).transpose(0, -1).reshape(corr_map.shape[1], -1,
                                                                                                   self.match_dim)
...
class DMModule(Module):
    ...
    def __setstate__(self, state):
        ...
        super(DMModule, self).__setstate__(state)
    ...
    def aia_attention_forward(
        ...
        # Calculate attention and out projection
        attn_output, attn_output_weights,attn1 = self.aia_scaled_dot_product_attention(q, k, v, attn_mask, dropout_p,
                                                                                 pos_emb)
        ...
        if need_weights:
            # Average attention weights over heads
            attn_output_weights = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
            attn1 = attn1.view(bsz, num_heads, tgt_len, src_len)
            return attn_output, attn_output_weights.sum(dim=1) / num_heads,attn1.sum(dim=1) / num_heads
        else:
            return attn_output, None
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None,
        ...
        if not self._qkv_same_embed_dim:
            ...
            attn_output, attn_output_weights,attn1 = self.aia_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
        ...
        if self.batch_first:
            return attn_output.transpose(1, 0), attn_output_weights,attn1
        else:
            return attn_output, attn_output_weights,attn1
        
# aia_attention = AiAModule(128, 8, dropout=0,batch_first=True,match_dim=128,feat_size=512).cuda()
# q = k =v= torch.randn((5,512,128)).cuda()
# pos = torch.randn((512,5,128)).cuda()
# tgt2 = aia_attention(q, k, v,pos_emb=pos)
