...
def initialize_optimizer(params, cfg):
    assert cfg['TRAINING']['optimizer'] in ['SGD', 'AdamW', 'Adagrad', 'Adam', 'RMSProp', 'Rprop', 'SGDLars']
...
def initialize_scheduler(optimizer, cfg, last_step=-1):
    if cfg['TRAINING']['lr_schedule'] == 'StepLR':
        ...
        return PolyLR(optimizer, max_iter=int(cfg['TRAINING']['max_iter']), power=float(cfg['TRAINING']['poly_power']),
                      last_step=last_step)
    elif cfg['TRAINING']['lr_schedule'] == 'onecycleLR':
        return OneCycleLR(optimizer, max_lr=float(cfg['TRAINING']['lr']), total_steps=int(cfg['TRAINING']['max_iter']))
