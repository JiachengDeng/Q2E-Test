...
CLASS_LABELS = ['ceiling', 'floor', 'wall', 'beam', 'column', 'window', 'door', 'chair', 'table', 'bookcase', 'sofa', 'board', 'clutter']
UNKNOWN_ID = -100
...
def confusion_matrix(pred_ids, gt_ids):
    ...
    idxs = gt_ids >= 0
    return np.bincount(pred_ids[idxs] * 13 + gt_ids[idxs], minlength=13*13).reshape((13, 13)).astype(np.ulonglong)
