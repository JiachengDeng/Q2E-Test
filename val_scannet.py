...
from scannet.scannet import ScanNetEvaluate, collate_fn_evaluate
import glob
...
def infer(model, dataloader, normalize_color=True, device=None):
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            # Feed forward
            start = time.time()
            preds_3d = model.forward_eval(sinput,coords_raw,superpoint)
            ...
            #sum_time += time.time() - start
            bar(f'time {sum_time/(i+1):.1f}', i + 1, len(dataloader))
...
def compute_miou(result, save_log=False):
    ...
    if len(pred_list[0].shape) >1:
        ...
        miou = iou.evaluate(np.hstack(pred_list), np.hstack(label_list), save_log=save_log)
...
if __name__ == '__main__':
    ...
    data_root = '/ssd/Dataset/ScanNetV2_seg/preprocess/validation'
    segment_root = data_root
    model_path1 = home_path1+'/pth/scannet.pth'
    voxel_size = 0.02
    num_workers = 0
    ...
    res = None
    model = AIA(in_channels=3,features=96,out_channels=20).cuda()
    ...
    for i in range(10):
        ...
        # out1 = infer(model, loader, True)
        # # #out2 = infer2(model2, loader, True)
        # for k in out.keys():
        #         out[k][0] += out1[k][0]
        if res is None:
            res = out
    ...
    for i, f in enumerate(res.keys()):
        ...
        if len(path_to_segment) > 0:
            segment = torch.tensor(np.load(path_to_segment[0][:-11]+f[5:]+'.npy')[:,9])
